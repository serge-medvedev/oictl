# channels-control Specification

## Purpose
TBD - created by archiving change add-channels-control. Update Purpose after archive.
## Requirements
...
#### Scenario: Unsupported channel list filters fail locally
- **WHEN** a user supplies `--page`, `--query`, `--q`, `--order-by`, or `--direction` to `oictl channels list`
- **THEN** the CLI exits non-zero before any request because the original channel inventory endpoint does not implement those filters
