...
type jsonInputHelp struct {
	usage, input, examples, source string
}
...
var jsonInputReferences = map[string]jsonInputHelp{
	"manifests diff":  {"manifests diff (--file PATH[,PATH...] | --directory DIR[,DIR...] | --dir DIR[,DIR...])", manifestInputHelp, manifestExamples, "internal/cli/manifests.go: loadManifestDocuments, manifestHandlers, manifestPayload, validateManifestPlan; models/{channels,functions,groups,knowledge,models,prompts,skills,tools}.py; routers/configs.py: TerminalServerConnection"},
	"manifests apply": {"manifests apply (--file PATH[,PATH...] | --directory DIR[,DIR...] | --dir DIR[,DIR...]) [--dry-run]", manifestInputHelp, manifestExamples, "internal/cli/manifests.go: loadManifestDocuments, manifestHandlers, manifestPayload, validateManifestPlan; models/{channels,functions,groups,knowledge,models,prompts,skills,tools}.py; routers/configs.py: TerminalServerConnection"},
	"manifests sync":  {"manifests sync (--file PATH[,PATH...] | --directory DIR[,DIR...] | --dir DIR[,DIR...]) --scope SCOPE [--dry-run] [--yes | --confirm]", manifestInputHelp, manifestExamples, "internal/cli/manifests.go: loadManifestDocuments, manifestHandlers, manifestPayload, syncScopeKinds; models/{channels,functions,groups,knowledge,models,prompts,skills,tools}.py; routers/configs.py: TerminalServerConnection"},
  ...
  Illustrative POST /api/v1/auths/signin body (email/password strings required):`, `{"email":"user@example.com","password":"example-password"}`, "app.go: runAPI, requestBody; routers/auths.py: signin (example only)"},
	"functions create": {"functions create", functionFieldsHelp, functionExample, "models/functions.py: FunctionForm, FunctionMeta; routers/functions.py: create_new_function"},
	"functions update": {"functions update <function-id>", functionFieldsHelp, functionExample, "models/functions.py: FunctionForm; routers/functions.py: update_function_by_id"},
  ...
  Entries are FunctionWithValvesModel records, not only creation forms:
...
{"functions":[{"id":"example_function","name":"Example Function","type":"pipe","content":"class Pipe:\n    def pipe(self, body: dict):\n        return 'Example'\n","meta":{},"created_at":0,"updated_at":0}]}`, "models/functions.py: FunctionWithValvesModel; routers/functions.py: SyncFunctionsForm, sync_functions; surfaces.go: functionSyncBody"},
  ...
  resource-defined by the function's Valves class. Inspect separately:
  ...
  with no required valves; it is not a guarantee for every function.`, `{}`, "routers/functions.py: update_function_valves_by_id; surfaces.go: runValves"},
  ...
  UserValves. Keys/types/defaults are resource-defined; no positional user ID.
  Inspect separately: oictl functions valves user spec <function-id>
  Server removes null values and stores explicitly supplied validated values.
  Example is illustrative for a resource with no required user valves.`, `{}`, "routers/functions.py: update_function_user_valves_by_id; surfaces.go: runValves"},
	"skills create": {"skills create [--manifest PATH]", skillFieldsHelp, skillExample, "models/skills.py: SkillForm, SkillMeta; routers/skills.py: create_new_skill; surfaces.go: skillPayloadBody, parseSkillManifest"},
	"skills update": {"skills update <skill-id> [--manifest PATH]", skillFieldsHelp, skillExample, "models/skills.py: SkillForm; routers/skills.py: update_skill_by_id; surfaces.go: skillPayloadBody"},
  ...
  This replaces grants only, not the full skill form.` + nativeGrantsHelp, `{"access_grants":[]}`, "routers/skills.py: SkillAccessGrantsForm, update_skill_access_by_id"},
	"tools create": {"tools create", toolFieldsHelp, toolExample, "models/tools.py: ToolForm, ToolMeta; routers/tools.py: create_new_tools"},
	"tools update": {"tools update <tool-id>", toolFieldsHelp, toolExample, "models/tools.py: ToolForm; routers/tools.py: update_tools_by_id"},
	"tools access-update": {"tools access-update <tool-id>", `  Required JSON object with required access_grants:array of grant objects.
  Replaces grants only, not tool source/metadata.` + nativeGrantsHelp, `{"access_grants":[]}`, "routers/tools.py: ToolAccessGrantsForm, update_tool_access_by_id"},
	"tools load-url": {"tools load-url", `  Required JSON object: url:string required, server validates an HTTP(S) URL.
  Server fetches tool source from it. No positional URL or field defaults.`, `{"url":"https://tools.example.com/example.py"}`, "routers/tools.py: LoadUrlForm, load_tool_from_url; surfaces.go: runTools"},
  ...
  to the selected tool's Valves class. Inspect separately:
  oictl tools valves spec <tool-id>
  Server removes null values and stores explicitly supplied validated values.
  Example is illustrative for a tool with no required valves, not a fixed schema.`, `{}`, "routers/tools.py: update_tools_valves_by_id; surfaces.go: runValves"},
	"providers openai config update": {"providers openai config update", openAIConfigHelp, openAIConfigExample, "routers/openai.py: OpenAIConfigForm, update_config, get_headers_and_cookies"},
	"providers openai config set": {"providers openai config set", openAIConfigHelp + `
  Existing alias of config update; identical body and replacement semantics.`, openAIConfigExample, "routers/openai.py: OpenAIConfigForm, update_config; surfaces.go: runProviders"},
	"providers ollama config update": {"providers ollama config update", ollamaConfigHelp, ollamaConfigExample, "routers/ollama.py: OllamaConfigForm, update_config"},
	"providers ollama config set": {"providers ollama config set", ollamaConfigHelp + `
  Existing alias of config update; identical input and replacement semantics.`, ollamaConfigExample, "routers/ollama.py: OllamaConfigForm, update_config; surfaces.go: runProviders"},
...
` + openAIOptionsHelp, `{"url":"https://provider.example.com/v1","key":"example-key","config":{}}`, "routers/openai.py: ConnectionVerificationForm, verify_connection"},
  ...
  Server checks the backend /api/version; no OLLAMA_API_CONFIGS envelope.`, `{"url":"https://ollama.example.com","key":null}`, "routers/ollama.py: ConnectionVerificationForm, verify_connection"},
	"providers openai request": {"providers openai request <path> [--method METHOD] [--header NAME:VALUE] [--stream]", `  Optional JSON body forwarded to the selected /openai endpoint. Endpoint and
  ...
  simple text example; additional message shapes/options belong to the provider.`, `{"model":"example-model","messages":[{"role":"user","content":"Hello"}],"stream":false}`, "surfaces.go: runProviders; routers/openai.py: generate_chat_completion (example)"},
	"providers ollama request": {"providers ollama request <path> [--method METHOD] [--header NAME:VALUE] [--stream]", `  Optional JSON body forwarded to the selected /ollama endpoint. Endpoint and
  ...
  prompt:string and stream:boolean are endpoint options (server stream default true).`, `{"model":"example-model","prompt":"Hello","stream":false}`, "surfaces.go: runProviders; routers/ollama.py: GenerateCompletionForm (example)"},
  ...
  user_ids. Standard channels (null type) require admin. Name is lowercased.`, `{"name":"example-channel","description":"Examples","is_private":false}`, "models/channels.py: CreateChannelForm, insert_new_channel; routers/channels.py: create_new_channel"},
  ...
  type is creation-only and not changed by this form.`, `{"name":"example-channel","description":"Updated examples","is_private":false}`, "models/channels.py: ChannelForm, update_channel_by_id; routers/channels.py: update_channel_by_id"},
  ...
  omission adds nobody for that selector. No nullable fields in this form.`, `{"user_ids":["user-a"],"group_ids":[]}`, "routers/channels.py: UpdateMembersForm, add_members_by_id"},
  ...
  Removes listed users; omission does not select all members. No group_ids field.`, `{"user_ids":["user-a"]}`, "routers/channels.py: RemoveMembersForm, remove_members_by_id"},
...
{"active":true}`, "routers/channels.py: UpdateActiveMemberForm; surfaces.go: normalizeChannelMemberActiveBody"},
  ...
  Server assigns message identity/timestamps; those are not required input.`, `{"content":"Example message","data":{"files":[]},"parent_id":null}`, "models/messages.py: MessageForm; routers/channels.py: post_new_message"},
  ...
  With no JSON, --name builds {"name":"..."}; explicit JSON takes precedence.`, `{"name":"thumbsup"}`, "routers/channels.py: ReactionForm; surfaces.go: runChannelReactions"},
  ...
  JSON or --name is required for creation.`, `{"name":"Example webhook","profile_image_url":null}`, "models/channels.py: ChannelWebhookForm; surfaces.go: runWebhookChannels, channelWebhookMutationBody"},
  ...
  JSON files. No JSON means convenience fields build the creation form.`, `{"name":"Example webhook"}`, "surfaces.go: ensureChannelWebhook; models/channels.py: ChannelWebhookForm"},
  ...
  fields and preserve omitted values. Supply JSON or a convenience field.`, `{"name":"Updated webhook","profile_image_url":null}`, "surfaces.go: runWebhookChannels, channelWebhookUpdateBody; models/channels.py: ChannelWebhookForm"},
...
` + eventWebhookFieldsHelp, `{"name":"Example events","url":"https://hooks.example.com/events","enabled":true,"events":["*"],"targets":[{"type":"user","id":"user-a"}]}`, "main.py: EventWebhookForm, create_event_webhook; events.py: normalize_event_webhook, normalize_event_targets"},
  ...
  Omitted fields retain existing values (exclude_unset); explicit null is applied:
  enabled:null becomes false, url:null becomes "", name:null resets the name.
` + eventWebhookFieldsHelp, `{"enabled":false,"events":["*"],"targets":null}`, "main.py: EventWebhookUpdateForm, update_event_webhook; events.py: normalize_event_webhook"},
	"groups create":       {"groups create", groupFieldsHelp, `{"name":"Example Team","description":"Example group","permissions":{"workspace":{"models":true}}}`, "models/groups.py: GroupForm; routers/groups.py: create_new_group"},
	"groups update":       {"groups update <group-id>", groupFieldsHelp, `{"name":"Example Team","description":"Updated group","permissions":{"workspace":{"models":true}}}`, "models/groups.py: GroupUpdateForm, update_group_by_id; routers/groups.py: update_group_by_id"},
	"groups users add":    {"groups users add <group-id> [<user-id>...]", groupMembersHelp, `{"user_ids":["user-a","user-b"]}`, "models/groups.py: UserIdsForm; surfaces.go: runGroupUsers"},
	"groups users remove": {"groups users remove <group-id> [<user-id>...]", groupMembersHelp, `{"user_ids":["user-a"]}`, "models/groups.py: UserIdsForm; surfaces.go: runGroupUsers"},
  ...
  Role is an unconstrained form string, not a CLI enum.`, `{"name":"Updated Example User","role":"user"}`, "models/users.py: UserUpdateForm; routers/users.py: update_user_by_id"},
  ...
  ui:null stores null. Upstream UserSettings also allows extra top-level keys.
  ui.toolServers requires --allow-sensitive-ui-keys in the CLI.
  Example is illustrative of current-user UI preferences, not a cross-user patch.`, `{"ui":{"theme":"dark"}}`, "models/users.py: UserSettings, update_user_settings_by_id; routers/users.py: update_user_settings_by_session_user; surfaces.go: userSettingsMutationBody"},
	"users ui-settings patch": {"users ui-settings patch <user-id> --allow-ui-settings-extension [--allow-sensitive-ui-keys]", uiPatchHelp, `{"theme":"dark"}`, "surfaces.go: runUsers, userSettingsMutationBody (deployment extension)"},
  ...
  The same direct UI object is sent to each selected user.`, `{"theme":"dark"}`, "surfaces.go: runUsersUISettingsBulkPatch, collectBulkUserIDs, bulkUISettingsTargetSourceFor"},
  ...
  Import creates records; CLI does not normalize bare arrays or NDJSON exports.`, `{"chats":[{"chat":{"title":"Example chat","models":[],"history":{"messages":{},"currentId":null}},"pinned":false}]}`, "models/chats.py: ChatForm, ChatImportForm, ChatsImportForm, import_chats; routers/chats.py: import_chats"},
  ...
  --model builds {"model":"..."}; explicit JSON wins over --model.`, `{"model":"example-model"}`, "routers/chats.py: CompactChatForm, compact_chat_by_id; surfaces.go: runChats"},
  ...
  Explicit JSON takes precedence over --tag.`, `{"name":"example"}`, "routers/chats.py: TagForm, add_tag_by_id_and_tag_name; surfaces.go: runChatTags"},
  ...
  Supply JSON or exactly one --tag to build the object; explicit JSON wins.`, `{"name":"example"}`, "routers/chats.py: TagForm, delete_tag_by_id_and_tag_name; surfaces.go: runChatTags"},
	"automations create": {"automations create", automationHelp, automationExample, "models/automations.py: AutomationForm, AutomationData, AutomationTerminalConfig; routers/automations.py: create_new_automation"},
	"automations update": {"automations update <automation-id>", automationHelp, automationExample, "models/automations.py: AutomationForm, update_by_id; routers/automations.py: update_automation_by_id"},
...
` + scimUserNestedHelp, `{"userName":"user@example.com","displayName":"Example User","emails":[{"value":"user@example.com","primary":true}],"active":true}`, "routers/scim.py: SCIMUserCreateRequest, SCIMName, SCIMEmail, SCIMPhoto, create_user"},
...
` + scimUserNestedHelp, `{"displayName":"Updated Example User","emails":[{"value":"user@example.com"}],"photos":null}`, "routers/scim.py: SCIMUserUpdateRequest, update_user"},
  ...
  add/remove and pathless operations are not implemented by this route.`, `{"Operations":[{"op":"replace","path":"active","value":false}]}`, "routers/scim.py: SCIMPatchRequest, SCIMPatchOperation, patch_user"},
...
` + scimGroupNestedHelp, `{"displayName":"Example Team","members":[{"value":"user-a"}]}`, "routers/scim.py: SCIMGroupCreateRequest, SCIMGroupMember, create_group"},
...
` + scimGroupNestedHelp, `{"displayName":"Example Team","members":[]}`, "routers/scim.py: SCIMGroupUpdateRequest, update_group"},
  ...
  Omitted fields not targeted by these operations retain existing values.`, `{"Operations":[{"op":"add","path":"members","value":[{"value":"user-a"}]}]}`, "routers/scim.py: SCIMPatchRequest, SCIMPatchOperation, patch_group"},
	"auth login": {"auth login [--save]", `  JSON object: required email:string and password:string. CLI permits no body,
  but upstream SigninForm requires both fields. --save saves the returned token;
  selecting --profile also saves it. No fields/defaults are inserted by the CLI.`, `{"email":"user@example.com","password":"example-password"}`, "models/auths.py: SigninForm; routers/auths.py: signin; app.go: runAuth"},
  ...
  Server validates profile image URLs. CLI permits no body; upstream requires it.`, `{"name":"Example User","profile_image_url":"/user.png","bio":null,"gender":null,"date_of_birth":null}`, "models/users.py: UpdateProfileForm; routers/auths.py: update_profile"},
  ...
  CLI permits no body; upstream requires both fields.`, `{"password":"old-example-password","new_password":"new-example-password"}`, "models/auths.py: UpdatePasswordForm; routers/auths.py: update_password"},
	"auth admin-config set": {"auth admin-config set", adminConfigHelp, adminConfigExample, "routers/auths.py: AdminConfig, update_admin_config"},
  ...
  CLI permits no body; upstream requires the object.`, `{"label":"Directory","host":"ldap.example.com","app_dn":"cn=reader,dc=example,dc=com","app_dn_password":"example-password","search_base":"dc=example,dc=com"}`, "routers/auths.py: LdapServerConfig, update_ldap_server"},
  ...
  CLI permits no body; upstream requires an object.`, `{"enable_ldap":false}`, "routers/auths.py: LdapConfigForm, update_ldap_config"},
	"auth oauth-config set": {"auth oauth-config set", oauthConfigHelp, `{"ENABLE_OAUTH":true,"OAUTH_PROVIDER_NAME":"Example"}`, "routers/auths.py: OAuthConfigForm, update_oauth_config"},
	"tasks config set":      {"tasks config set", taskConfigHelp, taskConfigExample, "routers/tasks.py: TaskConfigForm, update_task_config; app.go: runTasks"},
	"models create": {"models create", modelFieldsHelp + `
  CLI permits no body, but creation needs the model form. A supplied body must
  be a non-null object; omitted params is inserted as {} by the CLI.`, `{"id":"example-model","name":"Example Model","meta":{}}`, "models/models.py: ModelForm, ModelMeta, ModelParams; routers/models.py: create_new_model; app.go: normalizeModelParamsBody"},
  ...
  grants. This is not a general merge patch.`, `{"name":"Example Model","meta":{"description":"Updated"},"params":{}}`, "routers/models.py: update_model_by_id; models/models.py: update_model_by_id; app.go: bodyWithID"},
...
{"models":[{"id":"example-model","name":"Example Model","meta":{},"params":{}}]}`, "routers/models.py: ModelsImportForm, import_models; app.go: normalizeModelInventory"},
  ...
  Upstream SyncModelsForm consumes ModelModel records, NOT create forms.
...
{"models":[{"id":"example-model","user_id":"user-a","name":"Example Model","meta":{},"params":{},"is_active":true,"created_at":0,"updated_at":0}]}`, "routers/models.py: SyncModelsForm; models/models.py: ModelModel, sync_models; models/access_grants.py: AccessGrantModel; app.go: normalizeModelInventory"},
	"models toggle": {"models toggle <model-id>", `  Optional JSON object; the upstream form requires only id:string.
  The server toggles existing active state; this is not an is_active setter.` + modelIDHelp, `{}`, "routers/models.py: ModelIdForm, toggle_model_by_id; app.go: bodyWithID"},
	"models delete": {"models delete <model-id>", `  Optional JSON object; upstream requires only id:string and deletes that model.` + modelIDHelp, `{}`, "routers/models.py: ModelIdForm, delete_model_by_id; app.go: bodyWithID, deleteModel"},
  ...
  default null, used if the server must create a base-model entry.` + modelIDHelp + nativeGrantsHelp, `{"access_grants":[{"principal_type":"user","principal_id":"*","permission":"read"}]}`, "routers/models.py: ModelAccessGrantsForm, update_model_access_by_id; app.go: bodyWithID"},
  ...
  Example is illustrative of one UI configuration key.`, `{"config":{"ui.default_locale":"en"}}`, "routers/configs.py: ImportConfigForm, import_config; models/config.py: Config.upsert"},
  ...
  CLI permits no body; server requires the object.`, `{"ENABLE_DIRECT_CONNECTIONS":false,"ENABLE_BASE_MODELS_CACHE":true}`, "routers/configs.py: ConnectionsConfigForm, set_connections_config"},
...
` + toolServerFieldsHelp, `{"TOOL_SERVER_CONNECTIONS":[{"url":"https://tools.example.com","path":"/openapi.json","auth_type":"bearer","key":"example-key","config":{}}]}`, "routers/configs.py: ToolServersConfigForm, ToolServerConnection, set_tool_servers_config"},
...
` + toolServerFieldsHelp, `{"url":"https://tools.example.com","path":"/openapi.json","auth_type":"bearer","key":"example-key","config":{}}`, "routers/configs.py: ToolServerConnection, verify_tool_servers_config"},
...
` + terminalFieldsHelp, `{"TERMINAL_SERVER_CONNECTIONS":[{"id":"shell-a","name":"Shell","url":"https://terminal.example.com","config":{"access_grants":[]}}]}`, "routers/configs.py: TerminalServersConfigForm, TerminalServerConnection, set_terminal_servers_config"},
	"config terminal-servers verify": {"config terminal-servers verify", `  One connection object, not the full configuration envelope.
  CLI permits no body; server requires the form.
` + terminalFieldsHelp, `{"url":"https://terminal.example.com"}`, "routers/configs.py: TerminalServerConnection, verify_terminal_server_connection"},
  ...
  Example illustrates an empty policy document.`, `{"url":"https://terminal.example.com","policy_id":"example","policy_data":{}}`, "routers/configs.py: TerminalServerPolicyForm, put_terminal_server_policy"},
  ...
  CLI permits no body; upstream requires the form. Example is illustrative.`, `{"url":"https://terminal.example.com","policy_id":"example","lifecycle_data":{}}`, "routers/configs.py: TerminalServerLifecycleForm, put_terminal_server_lifecycle"},
  ...
  CLI permits no body; upstream requires the form.`, `{"url":"https://terminal.example.com","only_idle":true,"reset":false}`, "routers/configs.py: TerminalServerRefreshForm, refresh_terminal_server_terminals"},
...
{"access_grants":[]}
null`, "terminal_servers.go: decodeDesiredAccessGrants, runTerminalServerAccessGrants; manifests.go: validateAndNormalizeResolvedAccessGrants"},
...
{"access_grants":[]}
null`, "terminal_servers.go: decodeDesiredAccessGrants, runTerminalServerAccessGrants; manifests.go: validateAndNormalizeResolvedAccessGrants"},
	"config code-execution set": {"config code-execution set", codeConfigHelp, codeConfigExample, "routers/configs.py: CodeInterpreterConfigForm, set_code_execution_config"},
  ...
  CLI permits no body; upstream requires the object.`, `{"DEFAULT_MODELS":null,"DEFAULT_PINNED_MODELS":null,"MODEL_ORDER_LIST":[],"DEFAULT_MODEL_METADATA":{},"DEFAULT_MODEL_PARAMS":{}}`, "routers/configs.py: ModelsConfigForm, set_models_config"},
  ...
  CLI permits no body; upstream requires the object. There is no set subcommand.`, `{"suggestions":[{"title":["Example","Question"],"content":"Explain this topic."}]}`, "routers/configs.py: PromptSuggestion, SetDefaultSuggestionsForm, set_default_suggestions"},
  ...
  Full replacement list; CLI permits no body, upstream requires the form.`, `{"banners":[{"id":"notice","type":"info","content":"Example notice","dismissible":true,"timestamp":0}]}`, "routers/configs.py: SetBannersForm, set_banners; config.py: BannerModel"},
  ...
  CLI permits no body; upstream requires the form.`, `{"url":"https://tools.example.com","client_id":"example-client","oauth_scope":"openid"}`, "routers/configs.py: OAuthClientRegistrationForm, register_oauth_client"},
  ...
  Example illustrates metadata, not file bytes.`, `{"file_hash":"example-checksum","language":"en"}`, "app.go: multipartFileBody; routers/files.py: upload_file_handler, process_uploaded_file"},
  ...
  upstream ContentForm requires the object.`, `{"content":"Updated file text"}`, "routers/files.py: ContentForm, update_file_data_content_by_id"},
	"knowledge create":           {"knowledge create", knowledgeFieldsHelp, `{"name":"Example Knowledge","description":"Reference documents"}`, "models/knowledge.py: KnowledgeForm; routers/knowledge.py: create_new_knowledge"},
	"knowledge update":           {"knowledge update <knowledge-id>", knowledgeFieldsHelp, `{"name":"Example Knowledge","description":"Updated reference documents"}`, "models/knowledge.py: KnowledgeForm, update_knowledge_by_id; routers/knowledge.py: update_knowledge_by_id"},
	"knowledge reindex":          {"knowledge reindex", optionalForwardedHelp, `{}`, "routers/knowledge.py: reindex_knowledge_files; app.go: runKnowledge"},
	"knowledge metadata-reindex": {"knowledge metadata-reindex", optionalForwardedHelp, `{}`, "routers/knowledge.py: reindex_knowledge_base_metadata_embeddings; app.go: runKnowledge"},
	"knowledge get":              {"knowledge get <knowledge-id>", optionalForwardedHelp, `{}`, "routers/knowledge.py: get_knowledge_by_id; app.go: runKnowledge"},
	"knowledge export":           {"knowledge export <knowledge-id>", optionalForwardedHelp, `{}`, "routers/knowledge.py: export_knowledge_by_id; app.go: runKnowledge"},
	"knowledge delete":           {"knowledge delete <knowledge-id>", optionalForwardedHelp, `{}`, "routers/knowledge.py: delete_knowledge_by_id; app.go: runKnowledge"},
	"knowledge reset":            {"knowledge reset <knowledge-id>", optionalForwardedHelp, `{}`, "routers/knowledge.py: reset_knowledge_by_id; app.go: runKnowledge"},
  ...
  CLI permits no body; upstream requires this replacement form.` + nativeGrantsHelp, `{"access_grants":[]}`, "routers/knowledge.py: KnowledgeAccessGrantsForm, update_knowledge_access_by_id"},
	"knowledge files update": {"knowledge files update <knowledge-id>", knowledgeFileHelp, `{"file_id":"file-a"}`, "routers/knowledge.py: KnowledgeFileIdForm, update_file_from_knowledge_by_id"},
	"knowledge files remove": {"knowledge files remove <knowledge-id>", knowledgeFileHelp, `{"file_id":"file-a"}`, "routers/knowledge.py: KnowledgeFileIdForm, remove_file_from_knowledge_by_id"},
  ...
  CLI permits no body; upstream requires the object.`, `{"file_id":"file-a","directory_id":null}`, "routers/knowledge.py: KnowledgeFileMoveForm, move_file_in_knowledge"},
  ...
  Existing associations are skipped. CLI permits no body; server requires array.`, `[{"file_id":"file-a","directory_id":null}]`, "routers/knowledge.py: KnowledgeFileIdForm, add_files_to_knowledge_batch"},
  ...
  server default null (root). CLI permits no body; server requires the form.`, `{"name":"Examples","parent_id":null}`, "routers/knowledge.py: KnowledgeDirectoryCreateForm, create_knowledge_directory"},
  ...
  CLI permits no body; upstream requires an object.`, `{"name":"Updated Examples","parent_id":null}`, "routers/knowledge.py: KnowledgeDirectoryUpdateForm, update_knowledge_directory; models/knowledge.py: update_directory"},
	"knowledge dirs delete": {"knowledge dirs delete <knowledge-id> <dir-id>", optionalForwardedHelp + `
  Upstream move_files is a query parameter with default true, not a JSON field.`, `{}`, "routers/knowledge.py: delete_knowledge_directory; app.go: runKnowledgeDirs"},
  ...
  CLI permits no body; upstream requires this object. Diff does not upload files.`, `{"manifest":[{"filename":"example.txt","path":"docs/example.txt","checksum":"example-checksum","size":12}]}`, "routers/knowledge.py: FileManifestEntry, SyncDiffForm, sync_knowledge_diff; app.go: runKnowledgeSync"},
  ...
  CLI permits no body; upstream requires this object.`, `{"file_ids":["file-a"],"dir_ids":[]}`, "routers/knowledge.py: SyncCleanupForm, sync_knowledge_cleanup; app.go: runKnowledgeSync"},
  ...
  temp_id and reply/thread selectors are used on posting, not changed by update.`,
		...
		"models/messages.py: MessageForm, update_message_by_id; routers/channels.py: update_message_by_id",
	},
  ...
  by the selected tool's UserValves class, not a universal CLI schema.
  ...
  Example is illustrative for a resource with no required user valves.`,
		`{}`,
		"routers/tools.py: update_tools_user_valves_by_id; surfaces.go: runValves",
	},
	"users create": {
		"users create",
  ...
  A body source is required; the CLI does not insert these server defaults.`,
		...
		"models/auths.py: SignupForm, AddUserForm; routers/auths.py: add_user",
	},
}
...
func (a *App) printJSONInputHelp(args []string) bool {
	...
	for n := len(pos); n > 0; n-- {
		...
		fmt.Fprintf(a.out, "\nSource: Open WebUI 0aa65cd1c9d4, %s\n", ref.source)
		return true
	}
	return false
}
