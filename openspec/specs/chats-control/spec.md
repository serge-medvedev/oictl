## Purpose
Define chat record and chat workflow control commands for Open WebUI.
...
#### Scenario: User exports own chats
- **WHEN** a user runs `oictl chats export --out chats.ndjson`
- **THEN** the CLI requests `/api/v1/chats/all` for the authenticated user and preserves the server's NDJSON bytes
- **AND** the CLI exits successfully only after the output is written

#### Scenario: Admin explicitly exports all users' chats
- **WHEN** an admin runs `oictl chats export --all-users --out all-chats.json`
- **THEN** the CLI requests `/api/v1/chats/all/db`, subject to server authorization and admin-export configuration
- **AND** combining `--all-users` with `--chat-id` is rejected before any request

#### Scenario: Conflicting chat list scopes are rejected
- **WHEN** a user combines `--user-id`, `--archived`, or `--folder-id` in a way that selects incompatible server inventories
- **THEN** the CLI exits non-zero before any request instead of silently changing the selected user
